...
s03 的 `TodoManager` 只是記憶體中的扁平清單：**沒有順序、沒有依賴關係、狀態只有完成與未完成**。真實目標是有結構的——任務 B 依賴任務 A，任務 C 和 D 可以並行，任務 E 要等 C 和 D 都完成。此外，清單只存活在記憶體中，一旦上下文壓縮就消失無蹤。

### 解決方案

將扁平清單升級為持久化至磁碟的任務圖（DAG）。每個任務是一個 JSON 檔案，包含**狀態**、**前置依賴（`blockedBy`）**和**後置依賴（`blocks`）**。三個關鍵查詢：什麼可以執行？什麼被卡住？什麼已完成？透過自動依賴解除機制——完成任務時，自動將其 ID 從其他任務的 `blockedBy` 中移除，解鎖後續任務。
