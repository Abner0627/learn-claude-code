...
Agent 工作越久，`messages` 陣列就越龐大。每次讀取檔案、執行指令的輸出都永久留存在上下文中。「這個專案用什麼測試框架？」這類問題可能需要讀取 5 個檔案，但父 Agent 其實只需要一個答案：「pytest」。

### 解決方案

父 Agent 擁有一個 `task` 工具；子 Agent 擁有除 `task` 以外的所有基礎工具（**禁止遞迴生成**）。子 Agent 以空的 `messages=[]` 啟動，運行自己的迴圈。只有最終的文字結果會回傳給父 Agent，子 Agent 的完整對話歷史直接捨棄。父 Agent 收到的只是**一段精簡的摘要文字**，上下文負擔大幅降低。
