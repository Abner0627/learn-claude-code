...
### Q1：程式碼中的 `\033[33m` 是什麼意思？
**A：** 這是 **ANSI 轉義序列 (ANSI Escape Codes)**，用於控制終端機的文字格式。
*   `\033[33m`：開啟**黃色**文字模式。
*   `\033[0m`：**重設**格式，恢復終端機預設顏色。
*   中間的 `$ {block.input['command']}` 則是模型準備執行的指令，加上顏色是為了在終端機中清楚區分「模型的話」與「正在執行的指令」。

### Q2：為什麼系統給模型的回饋（工具執行結果）角色是 `user`？
**A：** 這是基於 **「模型 vs 外界」** 的二元邏輯。
...
### Q3：總結來說，除了模型自己的響應外，其餘都被視為 `user` 嗎？
**A：是的。** 只要是輸入給模型的資訊（無論來自人類或電腦程式），在對話協議中通通被視為 `user` 角色，這維持了清晰的「請求-回應」循環。

### Q4：隨著任務累積，`messages` 變龐大是否會拖垮模型表現？
**A：是的，這確實會帶來挑戰。** 隨著 Agent 執行更多步驟，Context 會不斷累積，主要影響有三點：
